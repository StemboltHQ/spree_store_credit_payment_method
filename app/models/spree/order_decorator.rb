Spree::Order.class_eval do
  state_machine.before_transition to: :confirm, do: :add_store_credit_payments
  state_machine.after_transition to: :confirm, do: :create_gift_cards
  ...
  def after_cancel_with_store_credits
    after_cancel_without_store_credits

    # Free up authorized store credits
    payments.store_credits.pending.each { |payment| payment.void! }

    # payment_state has to be updated because the original after_cancel does an
    # update_column on the payment_state to set it to 'credit_owed' but that is
    # not correct if the payments are captured store credits that get totally
    # refunded
  ...
  alias_method_chain :after_cancel, :store_credits
